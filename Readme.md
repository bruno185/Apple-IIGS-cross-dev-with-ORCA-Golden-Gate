...
## 2. Basic prerequisites on the PC
- **Windows 10 or 11**: The configuration has been tested on Windows 11, it is compatible with Windows 10.

- **Visual Studio Code**: The core of the proposed solution. Free, powerful, modular, extensible, easy to use, also available on Macintosh and Linux. The code editor is particularly powerful: http://www.visualstudio.com
...
| 5    | Copy to a disk image                   | Cadius                 |
| 6    | Test/run in the emulator               | KEGS or Crossrunner    |
| 7    | Test/run on a real Apple IIGS          | Apple IIGS             |
...
Copy to the disk image: Performed automatically by the script.

- Test in the emulator: Once the program is copied, it can be launched in KEGS or Crossrunner.
...
```python
#
# Python script to compile and deploy a program for Apple IIGS
...
4. Copy to the disk image: performed automatically by the script (Cadius).
5. Test in the emulator: once the program is copied, it can be launched in KEGS or Crossrunner.
...
Thanks to the "-i" option in the command line "iix rexport -i cadius {programname_name}", the "_FileInformation.txt" file is generated. It is required for the following command: "Cadius ADDFILE {programname_name}".
...
The C program passes parameters to assembly language functions, which then return values to the main program. These functions illustrate parameter passing between code in C compiled by ORCA/C and code in assembly compiled by ORCA/M.
The uppers function converts a C string variable to uppercase. It is compared with its C equivalent in terms of performance.
Finally, DEPLOY.py has been completed to generate the program listing "HelloAsmC.lst" (line 118), which is often necessary for debugging assembly code.

## Conclusion

This guide offers a complete method for cross-developing Apple IIGS applications on PC, relying on modern and accessible tools. By following these steps, you can efficiently automate compilation, disk image management, and testing of your programs.
